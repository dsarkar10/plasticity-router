...
class EnergyProfiler:
    def __init__(self, model: nn.Module, input_shape: tuple):
        self.model = model; self.input_shape = input_shape; self.macs_cache = {}

    def estimate_macs(self, strategy: str) -> float:
        if strategy in self.macs_cache: return self.macs_cache[strategy]
        base_forward = self._count_macs_forward(); base_backward = base_forward * 2
        costs = {"sgd": base_forward + base_backward, "er": base_forward + base_backward + base_forward * 0.5,
                 "ewc": base_forward + base_backward + base_forward * 1.5, "freeze": base_forward + base_backward * 0.4}
    ...
    def estimate_nps_routing_cost(self, num_tasks: int, num_freezes: int = 0) -> float:
        per_task = 2 * self.estimate_macs("sgd"); per_freeze = 2 * self.estimate_macs("sgd")
        return num_tasks * per_task + num_freezes * per_freeze

    def _count_macs_forward(self) -> float:
        self.model.eval(); dev = next(self.model.parameters()).device
        x = torch.zeros(1, *self.input_shape, device=dev); hooks = []
        def hook_fn(m, inp, out):
            if isinstance(m, nn.Linear): hooks.append(m.in_features * m.out_features)
...
class NPSComputer:
    def __init__(self, model: nn.Module, buffer: list, device: str = "cpu"):
        self.model = model; self.buffer = buffer; self.device = device; self.criterion = nn.CrossEntropyLoss()
    ...
    def compute_nps(self, x_new, y_new) -> float:
        ...
        self.model.zero_grad(); self.criterion(self.model(old_x), old_y).backward()
        grad_old = torch.cat([p.grad.view(-1) for p in self.model.parameters() if p.grad is not None])
        self.model.zero_grad(); self.criterion(self.model(x_new), y_new).backward()
        grad_new = torch.cat([p.grad.view(-1) for p in self.model.parameters() if p.grad is not None])
    ...
    def compute_layerwise_nps(self, x_new, y_new) -> dict:
        ...
        self.model.zero_grad(); x_new, y_new = x_new.to(self.device), y_new.to(self.device)
...
class EANPSPlugin(SupervisedPlugin):
    def __init__(self, input_shape: tuple, nps_threshold: float = 0.5,
        ...
        self.exp_counter = 0; self.disable_energy = disable_energy; self.disable_nps = disable_nps
    ...
    def _sample_balanced(self, n: int):
        ...
        groups = self._group_by_exp(); n_per = max(1, n // len(groups)); selected = []
        for indices in groups.values():
            k = min(n_per, len(indices)); selected.extend(np.random.choice(indices, k, replace=False).tolist())
    ...
    def before_training_exp(self, strategy, *args, **kwargs):
        ...
        high_nps = nps > self.nps_threshold; low_bat = self.battery < self.battery_critical
        energy = self.profiler.estimate_macs
        if self.disable_nps: high_nps = True
        if self.disable_energy: low_bat = False
        if not high_nps: strategy_name = "sgd"
        elif high_nps and not low_bat:
            er_cost, ewc_cost = energy("er"), energy("ewc")
            strategy_name = "er" if er_cost <= ewc_cost else "ewc"
        else: strategy_name = "freeze"
        self.current_strategy = strategy_name; self.strategy_history.append(strategy_name)
        ...
        if strategy_name == "freeze":
            ...
            for name, param in strategy.model.named_parameters(): param.requires_grad = name not in top_layers
    ...
    def after_training_exp(self, strategy, *args, **kwargs):
        ...
        if len(self.buffer) > self.mem_size:
            groups = self._group_by_exp(); n_per = self.mem_size // len(groups); kept = []
            for indices in groups.values():
...
def run_strategy(strategy_name, benchmark, seed, epochs_per_task=3,
                 mem_size=2000, batch_size=128, ea_battery=None, ea_decay=None):
    ...
    if ea_battery is not None: strategy.ea_plugin.battery = ea_battery
    if ea_decay is not None: strategy.ea_plugin.battery_decay = ea_decay
    ...
    for exp_idx, experience in enumerate(benchmark.train_stream):
        strategy.train(experience); model.eval(); accs = []
    ...
    if hasattr(strategy, 'ea_plugin'):
        plugin = strategy.ea_plugin; results["routes"] = plugin.strategy_history
...
def run_experiment(label, strategies, seeds, dataset_name="permuted_mnist"):
    print(f"\n{'#'*70}\n# {label}\n{'#'*70}")
    all_rows = []
    for seed in seeds:
        benchmark = PermutedMNIST(n_experiences=5, seed=seed)
        for strat in strategies:
            print(f"\n  {strat.upper()} | permuted_mnist | seed={seed}")
            res = run_strategy(strat, benchmark, seed, ea_battery=1.0, ea_decay=0.25)
            ...
            if res.get("routes"): row["routes"] = "\u2192".join(res["routes"])
            if res.get("macs_saved_pct") is not None: row["macs_saved_pct"] = res["macs_saved_pct"]
            if res.get("routing_macs") is not None: row["routing_macs"] = res["routing_macs"]
            if res.get("acc_matrix"): row["acc_matrix"] = str(res["acc_matrix"])
            if res.get("task_accuracies"): row["task_accuracies"] = str(res["task_accuracies"])
            all_rows.append(row)
...
print("  Full EA-NPS:      sgd\u2192er\u2192er\u2192freeze\u2192freeze")
print("  NPS-Only:         sgd\u2192er\u2192er\u2192er\u2192er")
print("  Energy-Only:      er\u2192er\u2192er\u2192freeze\u2192freeze")

df = run_experiment(
    "PermutedMNIST — Ablation fast decay, 3 seeds",
    strategies=["ea_nps", "ea_nps_nps_only", "ea_nps_energy_only"],
    seeds=[42, 43, 44],
)

df.to_csv("ablation_fast.csv", index=False)
print(f"\nSaved to ablation_fast.csv  (copy to vip_res/)")

summary = df.groupby("strategy").agg(
    acc_mean=("final_accuracy", "mean"), acc_std=("final_accuracy", "std"),
    time_mean=("time_seconds", "mean"), macs_saved=("macs_saved_pct", "first"),
    routes=("routes", "first"),
).round(4)
print(f"\n{'='*70}")
print("FINAL SUMMARY")
...
print(f"\n{'='*70}\nDone. Download ablation_fast.csv from Kaggle output.\n{'='*70}")
